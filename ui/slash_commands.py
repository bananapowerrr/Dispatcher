...
def list_slash_commands() -> list[str]:
    """Sorted command names for autocomplete (with leading /)."""
    return sorted(COMMANDS.keys())


def match_slash(prefix: str, *, limit: int = 12) -> list[str]:
    """prefix without or with leading /."""
    p = (prefix or "").strip().lower()
    if not p.startswith("/"):
        p = "/" + p
    return [c for c in list_slash_commands() if c.startswith(p)][:limit]
