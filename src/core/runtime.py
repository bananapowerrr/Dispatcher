...
class Runtime(RuntimeOps, RuntimeProcess):
    ...
    def process(self, raw: dict) -> str | None:
        ...
        try:
            ...
            if ch == "desktop" or (raw.get("metadata") or {}).get("primary_channel") == "desktop":
                ...
                try:
                    ...
                    _t = task_from_raw(raw, source="desktop", strict=True)
                except Exception:
        ...
        if not self.project_lock.acquire(project_key, tid, is_subtask=is_sub, parent_id=parent_id):
            ...
            try:
                ...
                task = task_from_raw(raw, source="runtime_busy", strict=False)
        ...
        if is_sub and files_lock:
            if not self.file_locks.acquire(files_lock, tid):
                ...
                try:
                    ...
                    task = task_from_raw(raw, source="runtime_file_busy", strict=False)
                    task.id = tid or task.id
        ...
        while True:
            try:
                ...
                try:
                    ...
                    raw = None
                if raw is None:
                    # desktop file-bus: deferred recover / stuck reclaim → incoming
                    try:
                        raw = self._claim_desktop_incoming()
                    except Exception as claim_exc:
                        try:
                            self.log.write(f"desktop claim: {claim_exc}")
                        except Exception:
                            pass
                        raw = None
                if raw is None:
                    # Optional phone/remote file-bus (gpt/grok/…)
                    try:
                        from core.feature_flags import is_enabled
    ...
    def _finalize_deduped(self, raw: dict, *, reason: str = "completed") -> None:
        """Write terminal done JSON so UI poll clears pending_ids (PC-26)."""
        tid = str((raw or {}).get("id") or "")
        if not tid:
            return
        channel = str((raw or {}).get("channel") or "desktop")
        try:
            self.bus.move(channel, "processing", "done", f"{tid}.json")
        except Exception:
            try:
                self.bus.move(channel, "incoming", "done", f"{tid}.json")
            except Exception:
                pass
        try:
            from core.intake_pipeline import task_from_raw
            task = task_from_raw(raw, source="dedupe", strict=False)
            task.id = tid
            ...
            self._save(
                ...
                "done",
                {
                    "method": "dedupe",
                    "worker": "dedupe",
                    "summary": f"дубликат пропущен ({reason})",
                    "stdout": f"DEDUPED:{reason}",
                },
            )
        except Exception as exc:
            try:
    ...
    def _maybe_defer_for_night(self, raw: dict) -> bool:
        ...
        try:
            ...
            task = task_from_raw(raw, source="night_defer", strict=False)
            task.id = tid or task.id
