...
def _cmd_status(args: list[str], ctx: dict[str, Any]) -> str:
    parts = []
    # PC-33: dispatcher + desktop queue (product path)
    try:
        from ui.dispatcher_ctl import is_running
        parts.append("dispatcher=" + ("ON" if is_running() else "OFF"))
    except Exception:
        try:
            from pathlib import Path as _P
            from ui.paths import agentbus_root
            pidf = agentbus_root() / ".agentbus" / "dispatcher_ui.pid"
            parts.append("dispatcher=" + ("pidfile" if pidf.is_file() else "unknown"))
        except Exception:
            pass
    try:
        from core.local_queue import get_local_queue
        from ui.paths import agentbus_root
        parts.append(f"queue={get_local_queue(agentbus_root()).size()}")
    except Exception:
        pass
    try:
