...
def _soft_log(msg: str) -> None:
    try:
        print(f"[reclaim] {msg}", file=sys.stderr, flush=True)
    except Exception:
        pass
...
def compute_stuck_timeout_sec(
    ...
    *,
    ...
    """Adaptive lease length for one task."""
...
def last_active_epoch(path: Path, raw: dict[str, Any] | None = None) -> float:
    ...
    for lf in (lease_json, lease):
        ...
        try:
            ...
            _soft_log(f"lease read {lf.name}: {type(exc).__name__}: {exc}")
