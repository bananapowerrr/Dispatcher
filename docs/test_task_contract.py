...
def test_normalize_blocks_path_traversal():
    with pytest.raises(TaskContractError, match="security"):
        normalize_task({"id": "a1", "message": "x", "files": ["../etc/passwd"]})


def test_normalize_blocks_dangerous_verify():
    with pytest.raises(TaskContractError, match="security"):
        normalize_task({"id": "a1", "message": "x", "verify": ["rm -rf /"]})


def test_normalize_safe_files_ok():
    t = normalize_task({"id": "a1", "message": "x", "files": ["src/main.py"]})
